...
export class ProductModalView extends View<IProduct> {
	...
	render(product: IProduct): HTMLElement {
		...
		(content.querySelector('.card__title') as HTMLElement).textContent =
			product.title;
		(content.querySelector('.card__text') as HTMLElement).textContent =
			product.description;
		(
			content.querySelector('.card__price') as HTMLElement
			...
			this.button.addEventListener('click', () => {
				if (product) {
					if (this.inCart) {
						...
						this.events.emit('cart:add', { product: product });
						this.setInCart(true);
					}
				}
			});
